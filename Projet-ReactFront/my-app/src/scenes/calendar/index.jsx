...
const Calendar = () => {
  ...
  const handleDateClick = async (selected) => {
    ...
    if (title) {
      const newEvent = await addReunion(authTokens?.accessToken,title, start, end, description, user.id);
      if (newEvent) {
  ...
  const handleEventClick = async (selected) => {
    ...
    if (action === "1") {
      ...
      if (updatedTitle) {
        ...
        try {
          await updateReunion(authTokens?.accessToken,user.id, eventId, {
            title: updatedTitle,
          ...
          const events = await getReunions(authTokens?.accessToken,user.id, start, end);
          console.log(user);
      ...
      if (
        ...
        const eventId = selected.event.id;
        try {
          await deleteReunion(authTokens?.accessToken,user.id, eventId);
          const calendarApi = selected.view.calendar;
          const updatedEvents = await getReunions(authTokens?.accessToken,user.id, calendarApi.view.activeStart, calendarApi.view.activeEnd);
          setCurrentEvents(updatedEvents);
        } catch (error) {
          console.log(error);
        }
