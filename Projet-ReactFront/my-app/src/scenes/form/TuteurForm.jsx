...
import { getTuteurById, updateTuteur } from "../../utils/api";
...
const FormTuteur = () => {
  ...
  const handleFormSubmit = async (values) => {
    try {
      await updateTuteur(id_tuteur, values, authTokens?.accessToken);
      enqueueSnackbar('Les modifications apportées au tuteur ont été enregistrées avec succès!', { variant: 'success' });
    } catch (error) {
      if (error.message) {
        enqueueSnackbar(error.message, { variant: 'error' });
      } else {
        enqueueSnackbar('Une erreur s\'est produite lors de la mise à jour du tuteur.', { variant: 'error' });
      }
    }
  };
  ...
  useEffect(() => {
    getTuteurById(id_tuteur, authTokens?.accessToken)
      .then((data) => setTuteurData(data))
      .catch((error) => console.log(error));
  }, [id_tuteur, authTokens?.accessToken]);
  ...
  return (
    <Box m="20px">
      <Header title="Modifier les informations du tuteur" subtitle="Transformez le compte du tuteur avec des informations à jour !" />
      <Formik
      ...
      >
        {({
          ...
          useEffect(() => {
            if (tuteurData) {
              const { nom, prenom, email, role, institution, entreprise } = tuteurData;
