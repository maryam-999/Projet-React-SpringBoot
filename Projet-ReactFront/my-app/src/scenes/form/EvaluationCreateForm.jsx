...
import { createEvaluation, getCompetencesByCategorie } from "../../utils/api";
...
const EvaluationCreateForm = () => {
    ...
    const handleRadioChange = (niveau, competenceId) => {
        const newValues = values.competenceEvaluations.map(item => {
    ...
    const handleFormSubmit = async (values) => {
        try {
            ...
            await createEvaluation(stageDto, authTokens?.accessToken);
            navigate('/TuteurDashboard/encadrement');
            enqueueSnackbar('Les ajouts ont été enregistrés avec succès!', { variant: 'success' });
        } catch (error) {
            navigate('/TuteurDashboard/encadrement');
            enqueueSnackbar(error.message || 'Erreur lors de l\'ajout.', { variant: 'error' });
        }
    };
    ...
    return (
        <Box m="20px">
            <Header title={`Évaluer le stagiaire ${data?.stagiaireNomComplet}`} subtitle="Saisissez les informations pour l'évaluation du stage." />
            <Stepper
                                                    ...
                                                    renderCell: (params) => (
                                                        <RadioGroup
                                                            ...
                                                            onChange={(e) => {
                                                                ...
                                                                if (existing) {
                                                                    existing.niveau = selectedValue;
                                                                } else {
                                                                    updated.push({ competenceId: params.row.id, niveau: selectedValue });
                                                                }
